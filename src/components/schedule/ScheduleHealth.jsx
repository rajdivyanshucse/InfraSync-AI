        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-surface-border bg-surface p-3 text-center min-w-[120px]">
              <span className="text-3xs font-mono uppercase text-slate-400 block">Baseline Target</span>
            ...
            <div className="rounded-xl border border-surface-border bg-surface p-3 text-center min-w-[120px]">
              <span className="text-3xs font-mono uppercase text-slate-400 block">Current Forecast</span>
              <span className="font-mono text-xs font-bold text-amber-400 mt-0.5 block">
                {scheduleMeta?.forecastCompletion || '12 Jan 2027'}
              </span>
            </div>

            <div className="rounded-xl border border-rose-500/30 bg-rose-950/20 p-3 text-center min-w-[110px]">
              <span className="text-3xs font-mono uppercase text-rose-300 block">Schedule Slip</span>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-surface-border bg-surface-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="rounded-lg bg-rose-950/50 p-1.5 text-rose-400 border border-rose-500/30">
          ...
          <div className="space-y-2 pt-1">
            {criticalPathActivities.slice(0, 4).map((act) => (
              <button
                ...
                className="w-full flex items-center justify-between rounded-lg bg-surface-subtle/70 p-2 text-left hover:bg-surface-elevated transition-colors group"
              >
                <div className="min-w-0 flex-1 pr-2">
                  <div className="flex items-center gap-1.5 font-mono text-3xs">
                    <span className="font-bold text-rose-400">{act.activityCode}</span>
                ...
                <div className="text-right font-mono text-3xs shrink-0">
                  <div className="text-slate-300">{act.actualProgress}%</div>
                  <div
                    className={
                      (act.variance ?? 0) < 0 ? 'text-rose-400 font-bold' : 'text-emerald-400'
                    }
                  >
                    {act.variance}%
                  </div>
        ...
        <div className="rounded-xl border border-surface-border bg-surface-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="rounded-lg bg-amber-950/50 p-1.5 text-amber-400 border border-amber-500/30">
          ...
          <div className="space-y-2 pt-1">
            {behindSchedule.slice(0, 4).map((act) => (
              ...
              >
                ...
                <div className="text-right font-mono text-3xs shrink-0">
                  <span className="rounded bg-amber-950/60 px-1.5 py-0.5 font-bold text-amber-400 border border-amber-500/30">
                    {act.variance}%
                  </span>
                </div>
              </button>
        ...
        <div className="rounded-xl border border-surface-border bg-surface-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="rounded-lg bg-brand-950/50 p-1.5 text-brand-400 border border-brand-500/30">
          ...
          <div className="space-y-2 pt-1">
            {upcomingMilestones.map((ms) => (
              <div
                key={ms.id}
                className="rounded-lg bg-surface-subtle/70 p-2.5 space-y-1.5 border border-surface-border/50"
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-3xs font-bold text-brand-400">
                    {ms.code}
                  </span>
                  <span className="font-mono text-3xs text-slate-400 flex items-center gap-1">
