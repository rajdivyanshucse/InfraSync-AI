    ...
    return (
      <div className={cn('relative flex items-center w-full', className)}>
        ...
        <input
          ...
          className={cn(
            'w-full h-9 rounded bg-surface-subtle border border-border pl-9 pr-16 text-sm text-foreground placeholder:text-foreground-muted/60 transition-all duration-150',
        ...
        <div className="absolute right-2.5 flex items-center gap-1.5">
          {value && onClear ? (
            ...
            <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 text-2xs font-mono font-medium text-foreground-muted bg-surface-muted/60 border border-border rounded">
              {shortcut}
            </kbd>
          ) : null}
        </div>
