      ...
      <div className="flex items-center justify-between border-b border-surface-border pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white tracking-wide">
            Rule Observation & Analysis Audit Timeline
          </h3>
        </div>
        ...
        {timelineEvents.map((event) => {
          ...
          return (
            <div key={event.id} className="relative space-y-2">
              ...
              {/* Event Card */}
              <div className="rounded-xl border border-surface-border bg-surface-subtle/40 p-3.5 space-y-2 hover:border-surface-border/80 transition-colors">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-2xs font-bold text-brand-300">
                      {event.id}
                    </span>
                    <span className="rounded bg-surface px-1.5 py-0.2 font-mono text-3xs text-slate-300 border border-surface-border">
                      {event.category}
                    </span>
                  </div>

                  <div className="flex items-center gap-1.5 font-mono text-3xs text-slate-400">
                ...
                {/* Scope & Trigger Row */}
                <div className="flex flex-wrap items-center justify-between border-t border-surface-border/40 pt-2 text-3xs font-mono text-slate-400 gap-2">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span>•</span>
                    <span>Trigger: <strong className="text-amber-300">{event.triggerRule?.split(':')[0]}</strong></span>
                  </div>
                  ...
                  <Button
                    ...
                    className="text-3xs h-6 px-2 text-brand-300 hover:text-white"
                  >
