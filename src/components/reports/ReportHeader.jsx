    ...
    <div className="rounded-xl border border-surface-border bg-gradient-to-b from-surface-card to-surface/80 p-5 shadow-lg backdrop-blur-sm">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-slate-400 sm:text-sm">
            Consolidated execution intelligence across schedule, ground progress, evidence, risk, and intervention.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 rounded-lg border border-surface-border bg-surface-subtle py-1 px-2.5">
            ...
            <select
              ...
              className="bg-transparent font-mono text-xs text-slate-200 focus:outline-none"
            >
              {REPORT_PERIODS.map((period) => (
                <option key={period.id} value={period.id} className="bg-surface-card text-white">
                  {period.label}
                </option>
              ))}
            </select>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-surface-border/60 pt-3 text-3xs font-mono text-slate-400">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <span className="flex items-center gap-1.5">
            Baseline Reference: <strong className="text-slate-200">{project?.code || 'INFRASYNC-P6'} Baseline v3.4</strong>
          </span>
          <span className="hidden text-slate-600 sm:inline">•</span>
          <span className="flex items-center gap-1.5">
            <FileCheck2 className="h-3 w-3 text-emerald-400" />
            Audit Trace: <strong className="text-slate-200">Phase 7–15 Data Fabric</strong>
          </span>
        </div>

        <div className="text-3xs text-slate-400 italic">
          * Prototype session preview. Real backend PDF generation planned for subsequent release.
        </div>
      </div>
    </div>
  );
