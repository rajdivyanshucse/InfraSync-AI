...
export const ContractorTable = ({
  ...
  const renderSortIcon = (field) => {
    ...
    return sortDirection === 'asc' ? (
      <ArrowUp className="ml-1 h-3 w-3 text-sky-500" />
    ) : (
      <ArrowDown className="ml-1 h-3 w-3 text-sky-500" />
    );
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-surface-border bg-surface-card p-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-sm">
          Try resetting filters or searching with a different trade keyword.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-surface-border bg-surface-subtle font-mono text-3xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
              <th
                ...
                className="group cursor-pointer px-4 py-3 font-semibold hover:text-slate-900 dark:hover:text-white min-w-[220px]"
              >
          ...
          <tbody className="divide-y divide-surface-border/60">
            {sortedList.map((item) => {
              const isSelected = selectedContractorId === item.id;
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded bg-sky-500/10 font-mono text-2xs font-bold text-sky-700 dark:text-sky-300 ring-1 ring-sky-500/20">
                        {item.code}
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-sky-700 dark:group-hover:text-sky-300 truncate max-w-[200px]">
                          {item.name}
                        </div>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">
                    <span className="inline-block rounded bg-surface-muted px-2 py-0.5 text-3xs font-medium text-slate-700 dark:text-slate-300 border border-surface-border truncate max-w-[180px]">
                      {item.category}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-3 text-center whitespace-nowrap font-mono text-xs font-semibold text-slate-800 dark:text-slate-200">
                    {item.wbsCount || (item.packageIds || []).length}
                  </td>
